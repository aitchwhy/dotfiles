...
class TodoistExporter:
    ...
    def get_or_create_project(self, project_name: str) -> str:
        ...
        projects = requests.get(
            f"{self.todoist_api}/projects", headers=self.headers
        ).json()
    ...
    def update_task(self, task_id: str, project_id: str, label_id: str) -> bool:
        ...
        try:
            # Get current task
            task = requests.get(
                f"{self.todoist_api}/tasks/{task_id}", headers=self.headers
            ).json()
            ...
            response = requests.post(
                ...
                headers=self.headers,
                json={"project_id": project_id, "labels": labels},
            )
            response.raise_for_status()
            return True
...
class TodoistExporterGUI:
    ...
    def create_token_section(self, parent):
        # Todoist Token
        ttk.Label(parent, text="Todoist API Token:").grid(
        ...
        ttk.Checkbutton(
            parent,
            text="Show",
            variable=self.todoist_show_var,
            command=lambda: self.toggle_token_visibility(
                self.todoist_token, self.todoist_show_var
            ),
    ...
    def create_button_section(self, parent):
        ...
        # Save Token Button
        ttk.Button(button_frame, text="Save Token", command=self.save_token).grid(
            row=0, column=0, padx=5
        )

        # Start Export Button
        ttk.Button(
            button_frame, text="Start Export & Organize", command=self.start_process
    ...
    def save_token(self):
        try:
            keyring.set_password(
                "todoist_exporter", "todoist_token", self.todoist_token.get()
            )
    ...
    def perform_process(self):
        try:
            ...
            export_filename = (
                f"todoist_url_tasks_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
