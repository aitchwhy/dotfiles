...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    home.activation.patchClaudeMcpExtensions = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
      ...
      ${pkgs.findutils}/bin/find "$EXT_DIR" -name "manifest.json" -type f -print0 | \
        ${pkgs.findutils}/bin/xargs -0 -I{} \
        ${pkgs.gnused}/bin/sed -i "" "s/\"command\": \"node\"/\"command\": \"$RUNTIME\"/g" {}

      # Patch .js shebangs (idempotent - only patches if node shebang found)
      ${pkgs.findutils}/bin/find "$EXT_DIR" -name "*.js" -type f -print0 | \
        ${pkgs.findutils}/bin/xargs -0 -I{} \
        ${pkgs.gnused}/bin/sed -i "" "s|#!/usr/bin/env node|#!/usr/bin/env $RUNTIME|g" {}
