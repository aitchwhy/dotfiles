...
let
  ...
  globalJustfileContent =
    ...
    concatStringsSep "\n" (
      [
        "# ─── Claude Code Multi-Account Launcher (cc) ────────────────────────────"
        "# Generated by Nix from accountDefs SSOT in modules/home/apps/claude.nix."
        ""
      ...
      ++ [
        ...
        ""
        "    # Drop any inherited CLAUDE_CODE_EFFORT_LEVEL so settings.json.effortLevel takes over."
        "    # Without this, stale ambient effort (e.g. from a prior shell) overrides in-session /effort."
...
in
{
  options.modules.home.apps.mcp = {
    ...
    # Expose `cc` justfile recipes for agents-launcher.nix to concatenate
    # with `cx` recipes from codex.nix into the single ~/.config/just/justfile.
    ccJustRecipes = lib.mkOption {
      type = lib.types.str;
      default = globalJustfileContent;
      readOnly = true;
      description = "Generated `cc` recipes (without shared header) for the global justfile";
    };
  };
    ...
    lib.mkMerge [
      {
        ...
        # NOTE: The global justfile (~/.config/just/justfile) is written by
        # modules/home/apps/agents-launcher.nix, which concatenates the `cc`
        # recipes exposed via `config.modules.home.apps.mcp.ccJustRecipes`
        # with `cx` recipes from `config.modules.home.apps.codex.cxJustRecipes`.
