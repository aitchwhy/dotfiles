...
fix_broken_links() {
  ...
  for dir in "$HOME" "$XDG_CONFIG_HOME" "$ZDOTDIR_TARGET"; do
    if [[ -d "$dir" ]]; then
      find -L "$dir" -type l -maxdepth 3 2>/dev/null | while read -r broken_link; do
        ...
        # Search in dotfiles directory for possible targets
        find "$DOTFILES" -name "$link_name" -type f 2>/dev/null | while read -r target; do
          possible_targets+=("$target")
        done
        
        if [[ ${#possible_targets[@]} -eq 1 ]]; then
        ...
        elif [[ ${#possible_targets[@]} -gt 1 ]]; then
...
update_shell_plugins() {
  ...
  if has_command pyenv; then
    info "Updating pyenv..."
    if has_command pyenv-update; then
      pyenv update
    else
...
# Check for ZDOTDIR changes and update accordingly
...
relink_configs() {
  ...
  if [[ -f "$DOTFILES/install.zsh" ]]; then
    ...
    # Run setup functions for symlinks
    setup_cli_tools
    setup_gui_apps
...
# Update macOS apps from the App Store
update_mac_apps() {
