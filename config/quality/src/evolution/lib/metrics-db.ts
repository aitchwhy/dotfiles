...
export interface MetricsDb {
  ...
  readonly updateTrend: (date: string, score: number) => Effect.Effect<void, DatabaseError>
  readonly getRecentGrades: (limit: number) => Effect.Effect<readonly GradeRecord[], DatabaseError>
  readonly getWeeklyTrends: (limit: number) => Effect.Effect<readonly TrendRecord[], DatabaseError>
}

export class MetricsDbService extends Context.Tag('MetricsDb')<MetricsDbService, MetricsDb>() {}
...
const makeMetricsDb = (db: Database): MetricsDb => ({
  storeGrade: (grade) =>
    Effect.try({
      ...
      catch: (cause) => new DatabaseError({ operation: 'storeGrade', cause }),
    }),
  ...
  updateTrend: (date, score) =>
    Effect.try({
      ...
      catch: (cause) => new DatabaseError({ operation: 'updateTrend', cause }),
    }),
  ...
  getRecentGrades: (limit) =>
    Effect.try({
      ...
      catch: (cause) => new DatabaseError({ operation: 'getRecentGrades', cause }),
    }),
  ...
  getWeeklyTrends: (limit) =>
    Effect.try({
      ...
      catch: (cause) => new DatabaseError({ operation: 'getWeeklyTrends', cause }),
    }),
})
