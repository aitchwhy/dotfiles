...
const checkTypeScript = Effect.gen(function* () {
  ...
  const untypedResult = yield* runCommand('grep', [
    '-r',
    '-c',
    UNTYPED_PATTERN,
    join(QUALITY_DIR, 'src'),
  ])
  const untypedCount =
    untypedResult.exitCode === 0 ? Number.parseInt(untypedResult.stdout.trim()) || 0 : 0

  const zinferResult = yield* runCommand('grep', [
    '-r',
    '-c',
    ZINFER_PATTERN,
    join(QUALITY_DIR, 'src'),
  ...
  const zinferCount =
    zinferResult.exitCode === 0 ? Number.parseInt(zinferResult.stdout.trim()) || 0 : 0
...
const checkHooks = Effect.gen(function* () {
  ...
  for (const hook of hooks) {
    ...
    if (!existsSync(hookPath)) {
      ...
      const grepResult = yield* runCommand('grep', [
        '-q',
        'process.stdin\\|export default',
        hookPath,
      ])
...
const checkParagon = Effect.gen(function* () {
  ...
  if (!existsSync(paragonGuard)) {
    return {
      ...
      score: 0,
      weight,
      details: { message: 'paragon-guard.ts missing', guard_count: 14, active_guards: 0 },
    }
  }
  ...
  const grepResult = yield* runCommand('grep', [
    '-c',
    'function check[A-Z]\\|const check[A-Z]',
    paragonGuard,
  ])
  const activeGuards =
    grepResult.exitCode === 0 ? Number.parseInt(grepResult.stdout.trim()) || 0 : 0
