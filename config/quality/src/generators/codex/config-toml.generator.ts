 ...
 * `modules/home/apps/codex.nix`, which substitutes `__REF_API_KEY__` and
 * writes the result to each `~/.codex-max-N/config.toml`.
 *
...
const emitHookEntry = (matcher: string | undefined, entry: CodexMatcherHookGroup['hooks'][number]): string => {
  const lines: string[] = []
  if (matcher !== undefined) {
    lines.push(`[[hooks.${entry.type === 'command' ? 'placeholder' : ''}]]`)
  }
  return lines.join('\n')
}
...
// Unused — kept as a placeholder for future hook-group-with-matcher-only emits
void emitHookEntry
...
const generateConfigToml = (): string => {
  ...
  lines.push('# Activation hook substitutes __REF_API_KEY__ from ~/.config/mcp/ref-api-key')
  lines.push('# before writing this to each ~/.codex-max-N/config.toml.')
  lines.push('')
