...
const cleanOldPlans = Effect.gen(function* () {
	const exists = yield* Effect.tryPromise(() =>
		fs.access(PLANS_DIR).then(() => true).catch(() => false)
	);
	if (!exists) return 0;
...
const isInNixShell = Effect.gen(function* () {
	...
	const nixShellInfo = yield* Effect.tryPromise(() =>
		fs.access("/tmp/.nix-shell-info").then(() => true).catch(() => false)
	);
...
const checkEnvironment = Effect.gen(function* () {
	...
	const hasPackageLock = yield* Effect.tryPromise(() =>
		fs.access(path.join(cwd, "package-lock.json")).then(() => true).catch(() => false)
	);
	if (hasPackageLock) {
	...
	const hasEnv = yield* Effect.tryPromise(() =>
		fs.access(path.join(cwd, ".env")).then(() => true).catch(() => false)
	);
	const hasEnvExample = yield* Effect.tryPromise(() =>
		fs.access(path.join(cwd, ".env.example")).then(() => true).catch(() => false)
	);
...
const checkEvolutionMetrics = Effect.gen(function* () {
	...
	const exists = yield* Effect.tryPromise(() =>
		fs.access(metricsPath).then(() => true).catch(() => false)
	);
...
const outputResult = (result: SessionStartOutput) =>
...
const main = Effect.gen(function* () {
	...
	const warnings = yield* checkEnvironment.pipe(
	...
	// Output result
	const output: SessionStartOutput = messages.length > 0
