...
const outputResult = (result: PostToolUseOutput) => Console.log(JSON.stringify(result));
...
const main = Effect.gen(function* () {
  ...
  // 4. Count generations
  const genCount = yield* countGenerations.pipe(Effect.catchAll(() => Effect.succeed(0)));
