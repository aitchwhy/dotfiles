...
import { type ContextSignal, getRulesByPriority, type TriggerRule } from './trigger-registry';
...
export const parseGitStatus = (statusOutput: string): NonNullable<Context['gitState']> => {
  ...
  return {
    ...
    hasUnstagedChanges: lines.some(
      (line) => line.startsWith(' M') || line.startsWith(' D') || line.startsWith('??')
    ),
