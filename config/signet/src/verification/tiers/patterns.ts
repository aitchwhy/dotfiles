...
import { PatternEngineLive, loadRulesFromDirectory, applyRules, detectLanguage } from '../../layers/patterns.js'
 ...
 */
const runQuickChecks = (content: string, filePath: string): { errors: number; warnings: number; details: string[] } => {
  let errors = 0
  let warnings = 0
...
export const runPatternsTier = (opts: VerificationOptions): Effect.Effect<TierResult, Error> =>
  Effect.gen(function* () {
    ...
    if (rulesExist) {
      const loadRulesEffect = loadRulesFromDirectory(rulesPath).pipe(Effect.provide(PatternEngineLive))

      const rules = yield* loadRulesEffect.pipe(
        ...
        for (const file of files.slice(0, 50)) {
          ...
          if (content) {
            ...
            const applyResult = yield* applyRules(content, language, rules, file).pipe(
              ...
              Effect.catchAll(() => Effect.succeed({ filePath: file, matches: [], hasErrors: false, hasWarnings: false }))
            )
