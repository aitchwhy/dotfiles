...
export interface PatternService {
  ...
  readonly parseSource: (content: string, language: PatternLanguage) => Effect.Effect<SgRoot, Error>
  ...
  readonly loadRulesFromDirectory: (dirPath: string) => Effect.Effect<readonly PatternRule[], Error>
...
const extractCaptures = (node: SgNode): Record<string, string> => {
  ...
  const commonVars = ['METHOD', 'NAME', 'PATH', 'HANDLER', 'ARGS', 'MSG', 'ERROR', 'APP', 'CONFIG', 'ARG', 'CTX', 'BODY']
...
const interpolateFix = (template: string, node: SgNode): string => {
  ...
  const vars = ['ARGS', 'METHOD', 'NAME', 'PATH', 'HANDLER', 'APP', 'CONFIG', 'ARG', 'CTX', 'BODY', 'MSG', 'ERROR']
 ...
 */
export const loadRuleFromYaml = (
  path: string
): Effect.Effect<PatternRule, Error, PatternEngine> =>
