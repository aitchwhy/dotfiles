 ...
 * @module daemon
 */
import { Context, Effect, Layer, Schedule, Duration, Ref, Console } from 'effect'
...
export const DaemonLive = (config: DaemonConfig) =>
  Layer.effect(
    Daemon,
    makeDaemon(config).pipe(Effect.provide(PulumiLive))
  )
...
export const DaemonTest = (config: DaemonConfig) =>
  Layer.effect(
    Daemon,
    makeDaemon(config).pipe(Effect.provide(PulumiTest))
  )
...
export { INITIAL_STATE, DEFAULT_CONFIG, EMPTY_PREVIEW } from './daemon/types'
