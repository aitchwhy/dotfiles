 ...
 * Run: bun config/agents/evolution/trend-alert.ts
 */
...
const analyzeAlerts = Effect.gen(function* () {
	...
	if (latest.avg_score < THRESHOLDS.critical) {
		alerts.push(`CRITICAL: Score ${Math.round(latest.avg_score * 100)}% below ${THRESHOLDS.critical * 100}%`);
		status = "critical";
	} else if (latest.avg_score < THRESHOLDS.warning) {
		alerts.push(`WARNING: Score ${Math.round(latest.avg_score * 100)}% below ${THRESHOLDS.warning * 100}%`);
		status = "warning";
	}
