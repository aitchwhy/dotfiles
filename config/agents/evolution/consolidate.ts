...
const SQL_DUMP_PATH = join(DOTFILES, 'config/agents/memory/lessons.sql');
...
function jaccardSimilarity(a: string, b: string): number {
  const tokensA = new Set(
    a.toLowerCase().split(/\s+/).filter(t => t.length > CONFIG.minTokenLength)
  );
  const tokensB = new Set(
    b.toLowerCase().split(/\s+/).filter(t => t.length > CONFIG.minTokenLength)
  );
...
function gitCommitDump(dumpPath: string): boolean {
  ...
  try {
    ...
    const commitResult = spawnSync(
      'git',
      ['commit', '-m', 'chore(evolution): archive lessons'],
      {
        cwd: DOTFILES,
        encoding: 'utf-8',
      }
