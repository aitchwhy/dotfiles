 ...
 * Run: bun config/agents/evolution/evolve.ts
 */

import { Effect, pipe } from "effect";
import { MetricsDbService, MetricsDbLive, type GradeRecord, type TrendRecord } from "./lib/metrics-db";
...
const color = (c: keyof typeof COLORS, text: string): string =>
...
const renderDashboard = (
	...
	Effect.gen(function* () {
		...
		if (grades.length > 0) {
			...
			yield* print(`│  Score: ${color(scoreColor, formatPercent(latest.overall_score))} ${color(scoreColor, makeBar(latest.overall_score))} │`);
			yield* print(`│  Status: ${color(scoreColor, latest.recommendation.toUpperCase().padEnd(10))}                              │`);
			yield* print(`│  Time: ${color("gray", latest.timestamp.slice(0, 19).padEnd(25))}             │`);
			...
			if (Object.keys(details).length > 0) {
				...
				for (const [name, result] of Object.entries(details)) {
					if (typeof result === "object" && result !== null && "score" in result) {
						...
						yield* print(`  ${label} ${color(sc, formatPercent(score).padStart(4))} ${color(sc, makeBar(score, 10))}`);
					}
				}
				yield* print("");
			}
		...
		if (trends.length > 0) {
			...
			for (const t of trends.slice(0, 7)) {
				...
				yield* print(`│  ${date}  ${color(sc, makeBar(t.avg_score, 25))} ${color(sc, formatPercent(t.avg_score).padStart(4))} │`);
			}
			yield* print(color("cyan", "└────────────────────────────────────────────────────┘"));
			yield* print("");
			...
			if (trends.length >= 2) {
				...
				if (Math.abs(delta) > 0.05) {
					...
					yield* print(`  Trend: ${color(trendColor, trend)} (${delta > 0 ? "+" : ""}${Math.round(delta * 100)}%)`);
					yield* print("");
				}
			}
		}
